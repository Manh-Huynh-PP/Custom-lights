...
class LIGHTING_PT_AddCustomLight(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        layout.operator("lighting.add_custom_light_plane", text="Gradient Light Plane", icon='MESH_PLANE')
...
class LIGHTING_PT_ManageLights(bpy.types.Panel):
    ...
    def draw_collection(self, layout, coll, name, is_master=False):
        ...
        # Add toggle viewport and render buttons for the collection
        # Check if any light in collection is hidden
        any_viewport_hidden = any(obj.hide_viewport for obj in objs)
        any_render_hidden = any(obj.hide_render for obj in objs)
        
        op_vp = row.operator("lighting.toggle_collection_viewport", text="", emboss=False, 
                            icon='HIDE_OFF' if not any_viewport_hidden else 'HIDE_ON')
        op_vp.collection_name = "MASTER" if is_master else coll.name
        
        op_rend = row.operator("lighting.toggle_collection_render", text="", emboss=False,
        ...
        if expanded:
            for obj in sorted(objs, key=lambda o: o.name):
                ...
                row_obj.alignment = 'LEFT'
                
                # Use old icon style: radio button for unselected, arrow for selected
                icon_obj = "KEYTYPE_MOVING_HOLD_VEC" if obj.select_get() else "RADIOBUT_OFF"

                op_obj = row_obj.operator("lighting.select_light", text=obj.name, emboss=False, icon=icon_obj)
                op_obj.light_name = obj.name
                
                # --- Controls on the same row as the name ---
                if obj.type == 'LIGHT':
                    row_obj.prop(obj.data, "type", text="")
                    if obj.data.type == 'AREA':
                        row_obj.prop(obj.data, "shape", text="")
                
                # Visibility Toggles (common for both types, but MESH has one extra)
                if obj.type == 'MESH' and hasattr(obj, "visible_camera"):
                    op_ray = row_obj.operator("lighting.toggle_ray_camera", text="", emboss=False, icon='RESTRICT_VIEW_OFF' if obj.visible_camera else 'RESTRICT_VIEW_ON')
                    op_ray.light_name = obj.name
                
                op_vp = row_obj.operator("lighting.toggle_viewport", text="", emboss=False, icon='HIDE_OFF' if not obj.hide_viewport else 'HIDE_ON')
                op_vp.light_name = obj.name
                
                op_rend = row_obj.operator("lighting.toggle_render", text="", emboss=False, icon='RESTRICT_RENDER_OFF' if not obj.hide_render else 'RESTRICT_RENDER_ON')
